...
const whichkey = logkey => {
  if (
    logkey.code === "ArrowUp" &&
    parseInt(sprite.style.bottom) <= 420 &&
    collisionDetect(sprite, boundingPlatforms, goalDetails) === true
  ) {
...
const gravityEffects = () => {
  if (parseInt(sprite.style.bottom) >= 1 && collisionDetect(sprite, boundingPlatforms, goalDetails) === false) {
    let heightString = sprite.style.bottom.replace("px", "");
    var height = parseInt(heightString, 10);
